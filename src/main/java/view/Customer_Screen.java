...
public class Customer_Screen extends JFrame {
    ...
    private Timer customerSyncTimer;
    private String activeCardKey = "EXPLORE_BOOK";
    ...
    private JPanel createCustomerSidebar() {
        ...
        btnLogout.addActionListener(e -> {
            ...
            if (confirm == JOptionPane.YES_OPTION) {
                if (customerSyncTimer != null && customerSyncTimer.isRunning()) {
                    customerSyncTimer.stop(); // Clean stop
                }
                this.dispose();
        ...
        sidebar.add(Box.createVerticalGlue());
        sidebar.add(createSidebarFooter());
    ...
    private JPanel createExploreAndBookPanel() {
        ...
        btnApplyFilter.setFont(new Font("Segoe Emoji UI", Font.BOLD, 12));
    ...
    private JPanel createSearchFieldGroup(String labelText, JComponent input) {
        ...
        lbl.setFont(new Font("Segoe Emoji UI", Font.BOLD, 9));
    ...
    public void refreshExploreRoomGrid() {
        ...
        List<CustomerDBA.RoomCardData> rooms = CustomerDBA.getAvailableRoomCardsForDates(
                searchCheckIn,
                searchCheckOut,
                searchGuestCount
        );
        ...
        if (rooms.isEmpty()) {
            ...
            JLabel lblEmpty = new JLabel("🚫 No Suites Available for " + searchGuestCount + " Guest(s) between " + searchCheckIn + " and " + searchCheckOut);
            lblEmpty.setFont(new Font("Segoe UI Emoji", Font.BOLD, 15));
            lblEmpty.setForeground(new Color(71, 85, 105));
            lblEmpty.setAlignmentX(Component.CENTER_ALIGNMENT);

            JLabel lblSub = new JLabel("All suites meeting this party size and date range are booked. Please try alternative dates or room capacities.");
    ...
    private JPanel createHousekeepingRequestPanel() {
        ...
        JButton btnSubmit = new JButton("✨ Dispatch Request") {
            @Override
            protected void paintComponent(Graphics g) {
                Graphics2D g2 = (Graphics2D) g.create();
                g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
    ...
    private JPanel createMyBillPanel() {
        ...
        btnPayFolio = new JButton("💳 Pay Bill (+ Earn Loyalty Points)") {
            @Override
            protected void paintComponent(Graphics g) {
                Graphics2D g2 = (Graphics2D) g.create();
                g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
        ...
        btnPayFolio.addActionListener(e -> {
            ...
            if (!bill.hasActiveStay || bill.netPayable.compareTo(BigDecimal.ZERO) <= 0) {
            ...
            if (ok) {
                String message = String.format(
                        "Payment Settled Successfully!\nTotal Points: %,d have been added to your Rewards.",
                        earnedPoints
                );
                JOptionPane.showMessageDialog(this, message, "Payment Completed", JOptionPane.INFORMATION_MESSAGE);

                refreshCustomerSession();
            } else {
                JOptionPane.showMessageDialog(this, "Transaction could not be completed. Please try again.", "Payment Error", JOptionPane.ERROR_MESSAGE);
            }
        });
    ...
    private void openChangePasswordDialog() {
        ...
        // Wrap each field with the interactive eye toggle component
        addGuestFormGroup(form, "Current Password", createPasswordFieldWithToggle(txtCurrent));
        addGuestFormGroup(form, "New Password", createPasswordFieldWithToggle(txtNew));
        addGuestFormGroup(form, "Confirm New Password", createPasswordFieldWithToggle(txtConfirm));
    ...
    private JPanel createPasswordFieldWithToggle(JPasswordField pwdField) {
        ...
        wrapper.setMaximumSize(new Dimension(1400, 36));
        wrapper.setPreferredSize(new Dimension(1400, 36));
        wrapper.setBorder(new EmptyBorder(0, 10, 0, 8));

        pwdField.setFont(new Font("Century Gothic", Font.PLAIN, 12));
        pwdField.setForeground(new Color(15, 23, 42));
        ...
        JComponent eyeToggle = new JComponent() {
            ...
            protected void paintComponent(Graphics g) {
                ...
                eyePath.moveTo(cx - 8, cy);
                eyePath.quadTo(cx, cy - 5.5, cx + 8, cy);
                eyePath.quadTo(cx, cy + 5.5, cx - 8, cy);
                g2.draw(eyePath);
        ...
        wrapper.add(pwdField, BorderLayout.CENTER);
    ...
    private JPanel createSidebarFooter() {
        ...
        footer.setMaximumSize(new Dimension(Integer.MAX_VALUE, 130));
        ...
        JLabel lblHeading = new JLabel("CONCIERGE & SUPPORT");
        lblHeading.setFont(new Font("Century Gothic", Font.BOLD, 9));
        lblHeading.setForeground(new Color(100, 116, 139));
        lblHeading.setAlignmentX(Component.LEFT_ALIGNMENT);

        JLabel lblPhone = new JLabel("📞  +95 9 798 123 456");
        lblPhone.setFont(new Font("Segoe UI Emoji", Font.PLAIN, 11));
        lblPhone.setForeground(new Color(203, 213, 225));
        lblPhone.setAlignmentX(Component.LEFT_ALIGNMENT);

        JLabel lblAddress = new JLabel("📍  Pyay Road, Dagon, Yangon");
        lblAddress.setFont(new Font("Segoe UI Emoji", Font.PLAIN, 10));
        lblAddress.setForeground(new Color(148, 163, 184));
        lblAddress.setAlignmentX(Component.LEFT_ALIGNMENT);

        JPanel socialRow = new JPanel(new FlowLayout(FlowLayout.LEFT, 8, 0));
        socialRow.setOpaque(false);
        socialRow.setAlignmentX(Component.LEFT_ALIGNMENT);

        JButton btnFb = createSocialIconButton("fb", "Facebook", new Color(59, 89, 152), "https://facebook.com");
        JButton btnInsta = createSocialIconButton("ig", "Instagram", new Color(225, 48, 108), "https://instagram.com");

        socialRow.add(btnFb);
        socialRow.add(btnInsta);

        footer.add(sep);
        footer.add(Box.createRigidArea(new Dimension(0, 10)));
        footer.add(lblHeading);
        footer.add(Box.createRigidArea(new Dimension(0, 6)));
        footer.add(lblPhone);
        footer.add(Box.createRigidArea(new Dimension(0, 3)));
        footer.add(lblAddress);
        footer.add(Box.createRigidArea(new Dimension(0, 8)));
        footer.add(socialRow);

        return footer;
    }

    private JButton createSocialIconButton(String platform, String tooltip, Color brandColor, String linkUrl) {
        JButton btn = new JButton() {
            ...
            protected void paintComponent(Graphics g) {
                ...
                g2.fill(new RoundRectangle2D.Double(0, 0, w, h, 8, 8));
                ...
                if ("fb".equalsIgnoreCase(platform)) {
                    ...
                    FontMetrics fm = g2.getFontMetrics();
                    int tx = (w - fm.stringWidth("f")) / 2;
                    int ty = ((h - fm.getHeight()) / 2) + fm.getAscent() - 1;
                    g2.drawString("f", tx, ty);
                } else if ("ig".equalsIgnoreCase(platform)) {
                    g2.setStroke(new BasicStroke(1.4f));
                    g2.drawRoundRect(8, 7, 16, 16, 5, 5);
                    g2.drawOval(12, 11, 8, 8);
                    g2.fillOval(20, 9, 2, 2);
                }
        ...
        btn.setMinimumSize(new Dimension(32, 30));
        ...
        btn.addActionListener(e -> {
            try {
                ...
                JOptionPane.showMessageDialog(this, tooltip + ": " + linkUrl, "Social Link", JOptionPane.INFORMATION_MESSAGE);
            }
        });
    ...
    private void startCustomerAutoSync() {
        customerSyncTimer = new Timer(4000, e -> {
            SwingUtilities.invokeLater(() -> {
                refreshCustomerSession();

                switch (activeCardKey) {
                    case "EXPLORE_BOOK":
                        if (roomCardsGridContainer != null) {
                            refreshExploreRoomGrid();
                        }
                        break;
